            const overlay = document.getElementById('scene-overlay') || document.createElement('div');
            if (!overlay.id) { overlay.id = 'scene-overlay'; document.body.appendChild(overlay); }
            const captionBar = document.querySelector('.photo-caption-bar') || document.createElement('div');
            if (!captionBar.className) { captionBar.className = 'photo-caption-bar'; document.body.appendChild(captionBar); }
            const descPanel = document.getElementById('photo-description');
            let ghost = null;
            let activeImg = null;
            ...
            function collapse() {
                ...
                ghost.addEventListener('transitionend', function() {
                    ...
                    if (activeImg) { activeImg.style.objectFit = ''; activeImg.style.height = ''; activeImg = null; }
                }, { once: true });
            }
            ...
            document.querySelectorAll('.photo-item img').forEach(img => {
                img.addEventListener('click', function(e) {
                    ...
                    const scale = Math.min(vw * 0.85 / r.width, vh * 0.70 / r.height);
                    const ghostCenterY = vh * 0.45;
                    const tx = (vw/2) - (r.left + r.width/2);
                    const ty = ghostCenterY - (r.top + r.height/2);
                    const finalH = r.height * scale;
